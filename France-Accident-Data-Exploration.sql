...
[CONTRIBUTING FACTOR VEHICLE 5]) as Factors
	...
	([BOROUGH]),
	([ACCIDENT DATE])
	,count([NUMBER OF PERSONS INJURED])  over (partition by [BOROUGH])sum_of_persons_injured_per_week
	FROM PortfolioProject..[nypd-motor-vehicle-collisions]
	where ([ACCIDENT DATE]) between '2012-07-01' and '2013-07-01' 
	Order by  3 desc ;


	---Using Temporary Table ( Subqueries and common table expressions CTE )
	WITH CTE_ACCIDENT AS (
	SELECT ([BOROUGH]),([ACCIDENT TIME]),[LOCATION]
		,COUNT([NUMBER OF PERSONS KILLED]) over (partition by [LOCATION]) as count_death
		,SUM([NUMBER OF PERSONS INJURED]) over (partition by [ACCIDENT TIME]) as sum_injured
...
---ROW NUMBER FUNCTION TO REMOVE DUPLICATES
	WITH DUPLICATES AS (
	SELECT[COLLISION_ID],Row_number () over (partition by [COLLISION_ID] ORDER by [COLLISION_ID] desc) as duplicate_death 
	
		
		FROM PortfolioProject..[nypd-motor-vehicle-collisions]
		)
		--Delete from DUPLICATES where duplicate_death=2


		SELECT *
		FROM DUPLICATES
		order by duplicate_death desc;